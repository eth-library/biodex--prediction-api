...
class PredModel(models.Model):
    # metadata records for Tensorflow Models used for predictions
    ...
    species_key_map = JSONField(blank=False, null=False) #save json as text for dev purposes
    #class_hierarchy_map: maps how classes in each hierarchical level map to the classes in their parent level
    class_hierarchy_map = JSONField(blank=False, null=False) #save json as text for dev purposes
    #encoded_hierarchy: array with all levels of the hierarchy encoded. Used for sorting
    encoded_hierarchy = JSONField( blank=False, null=False) #save json as text for dev purposes
